...
we call a valuation ``discrete'' when it is $\wzmZZ$-valued and \emph{also} normalized, encoding this notion in the following class\href{https://github.com/LCFT-Lean/local_fields/blob/76ad487d09babdb0018f394a5634526637ee014a/src/discrete_valuation_ring/basic.lean#L125}{\extlink}:
...
Since any uniformizer is necessarily a member of the unit ball, we decided to make the field $\pi$ in \mi{\st{the} this} definition \mi{\st{below}} a term of type \code{vR.integer} (as opposed to type \code{R}). \mi{\st{The bundled definition is more convenient to prove results that involve fixing a uniformizer.}}
\begin{lstlisting}
...
\mi{We use the proposition} \code{is_uniformizer} \mi{when proving lemmas that apply to any uniformizer element, such as the lemma stating that any uniformizer is nonzero:}
\begin{lstlisting}
lemma uniformizer_ne_zero {$\pi$ : R}
  (h$\pi$ : is_uniformizer vR $\pi$) : $\pi \ne 0$ := 
\end{lstlisting}

\mi{By contrast, the bundled definition is more convenient to prove results that involve fixing a uniformizer. For example, we use it when proving that every nonzero $r : \ball{K}$ can be factor as the product of a unit by a power of a fixed uniformizer:}
\begin{lstlisting}
...
Under this assumption, the typeclass inference system gets into an infinite loop when searching for an instance of
}
\code{
valuation_ring hv.v.valuation_subring.to_subring,}
\mi{because one of the first results that Lean tries to apply is}  \code{valuation_ring.of_field}, \mi{which reduces the problem to finding an instance of}
\code{field hv.v.valuation_subring.to_subring.}
\mi{This leads to a dead end (since the valuation subring is not a field); however, while trying to find this instance, the inference system finds} \code{normed_field.to_field}, \mi{and hence it starts to search for an instance of}
...
However, observe that no problem arises when putting both a \code{valued} and a \code{normed_field} instance on a specific field, like $\QQ_p$\mi{, because the typeclass inference system is able to find these instances on the specific field, and it will not try to apply them to find instances in other types.}

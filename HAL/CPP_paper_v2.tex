...
\code{normed_field hv.v.valuation_subring.to_subring}, for which it will try to apply the definition \code{discretely_normed_field}, which as input requires a \code{field} instance on the valuation subring \fil{which recreates the problem discussed above, risulting in \st{. Hence, }}the system get\fil{\st{s}ting} stuck in an infinite loop.

...
$ v_\m: K \to n_\m^{\mathbb{Z} \cup \{-\infty\}} = n_\m^\mathbb{Z} \cup \{0\}$ sending $r$ to $n_\m^{-\text{val}_v(r)}$ is a valuation on $K$, which we call the \textit{$\m$-adic valuation}. For example, when $R = \ZZ$, $K = \QQ$ and $\m = (p)$ is the ideal generated by a prime number $p$, $v_p$ is the $p$-adic (multiplicative) valuation on $\QQ$, also known as the $p$-adic absolute value.

We will represent valuations in Lean using the \href{https://leanprover-community.github.io/mathlib_docs/ring_theory/valuation/basic.html#valuation}{valuation} structure available in mathlib, which agrees with our above definition. A valuation on a ring $R$ induces a topology on $R$ for which the ring operations are continuous, hence turning $R$ into a topological ring.

Mathlib also provides the class \href{https://leanprover-community.github.io/mathlib_docs/topology/algebra/valuation.html#valued}{valued}, which bundles together a ring with a distinguished valuation and the uniform space structure induced by this valuation. This class is designed for rings in which there is a canonical valuation. For example, on the rational number $\QQ$ there are 

...
Let $R$ be a ring \mi{and let $\Gamma_0$ be a linearly ordered commutative monoid with zero, that is, a monoid with a partial order in which any two elements are comparable, and an added zero element that is smaller than all the others}. A \emph{valuation} $v$ on $R$ is a map $v \colon R \to \Gamma_0$ such that
\begin{enumerate}
	\item $v (0) = 0$,
	\item $v (1) = 1$,

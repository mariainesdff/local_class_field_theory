...
lemma antitone_unit_ball_pow [mixed_char_local_fi-- def normalized_valuation (K : Type*) [field K] [mixed_char_local_field p K] : valuation K ℤₘ₀ :=
--   (open_unit_ball K).valuation

-- instance (K : Type*) [field K] [mixed_char_local_field p K] : valued K ℤₘ₀ :=
--   valued.mk' (normalized_valuation K)sorry
--sorry eld p K] :
...
begin
  ...
  { have : ∀ n : ℕ, 1 ≤ n → (1 : 𝓞 p K) ∉ (unit_open_ball p K)^n,
    ...
    { use 0,
      rintros n ⟨hn, -⟩,
      by_contra' h_abs,
      replace h_abs : 1 ≤ n := nat.one_le_iff_ne_-- def normalized_valuation (K : Type*) [field K] [mixed_char_local_field p K] : valuation K ℤₘ₀ :=
--   (open_unit_ball K).valuation

-- instance (K : Type*) [field K] [mixed_char_local_field p K] : valued K ℤₘ₀ :=
--   valued.mk' (normalized_valuation K)sorry
--sorry zero.mpr (ne_of_gt h_abs),
      exact (this n h_abs) hn},
  },
  rw unit_open_ball,
